...
def main():
    ...
    for cfg_id in range(options.cfg_i,options.cfg_f,options.cfg_step):
        ...
        for flavor, beta, quarktype in [('light', 0.022, 'ud'),
                                        ('strange', 0.000, 's')]:
            cfg_dir = os.path.join('ini','cnfg{:02d}'.format(cfg_id))
            ...
            outfile = '../outputs/run_{:04d}.out'.format(cfg_id)
            ...
            for obj in run_objects:
                ...
                if obj.startswith('chroma'):
                    # os.makedirs(os.path.join(rundir,obj), exist_ok=True)
                ...
                if dataMap.get(f"run_{obj}",True):
                    ...
                    # if obj.startswith('chroma'):
                    #     with open(os.path.join(obj_dir,ini_out), 'w') as f:
                    #         f.write(output_xml)
                    # else:
                    with open(os.path.join(obj_dir,ini_out), 'w') as f:
                        f.write(output_xml)
